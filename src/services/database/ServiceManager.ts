...
import { BaseService, ServiceStatus, HealthCheckResult } from '../../database';
...
export class ServiceManager {
  ...
  async createService(schemaName: string, tableName: string): Promise<BaseService> {
    console.log(`[ServiceManager] Creating service for ${schemaName}:${tableName}`);
    ...
    try {
      ...
      if (config.autoInit) {
        console.log(`[ServiceManager] Service initing`);
        await service.init();
        console.log(`[ServiceManager] Service init done`);
      }
      ...
      console.log(`[ServiceManager] Service created, autoInit: ${config.autoInit}`);
      return service;
    } catch (error) {
      ...
      console.error(`[ServiceManager] Create service error for ${schemaName}:${tableName}:`, (error as Error).stack);
      throw error;
    }
  }
  ...
  async initService(schemaName: string, tableName: string): Promise<BaseService> {
    console.log(`[ServiceManager] Initializing service for ${schemaName}:${tableName}`);
    ...
    console.log(`[ServiceManager] service Initialized for ${schemaName}:${tableName}`);
    return service;
  }
