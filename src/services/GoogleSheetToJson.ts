...
import GoogleSheetFetcher, {
  ...
  SheetData,
  ProgressCallback,
} from './GoogleSheetFetcher'; // Import class đã tạo trước đó
...
export class GoogleSheetToJson {
   ...
   * @returns Promise chứa dữ liệu JSON theo từng table
   */
  public async getDataDirect(
    params: DirectGoogleSheetParams,
    ...
    try {
      ...
      if (sheetsArray.length === 0) {
        throw new GoogleSheetServiceError(
          'No valid sheets specified in tableList',
        );
      }
  ...
  private validateDirectParams(params: DirectGoogleSheetParams): void {
    ...
    if (params.timeout !== undefined && params.timeout <= 0) {
      throw new GoogleSheetServiceError('timeout must be a positive number');
    }
  ...
  private processDirectData(
    ...
    Object.keys(fetchResult.data).forEach((sheetName: string) => {
      ...
      if (Array.isArray(sheetData) && sheetData.length > 0) {
        ...
        console.log(
          `📊 Sheet "${sheetName}": Filtered ${filteredData.length} rows starting from row ${startFromRow} (original: ${sheetData.length} rows)`,
        );
      } else {
...
export const exampleUsage = {
  ...
  directFetch: async (): Promise<ProcessedData | null> => {
    try {
      const params: DirectGoogleSheetParams = {
        ...
        onProgress: progress => {
          console.log(
            `Download progress: ${progress.percent}% (${progress.loaded}/${progress.total})`,
          );
        },
      };

      const result: ProcessedData = await googleSheetService.getDataDirect(
        params,
      );
  ...
  withHelpers: async (): Promise<ProcessedData | null> => {
    try {
      ...
      // Validate URL first
      const isValid =
        googleSheetService.validateGoogleSheetUrl(googleSheetLink);
      if (!isValid) {
        throw new GoogleSheetServiceError('Invalid Google Sheet URL');
      }

      // Get available sheets
      const availableSheets = await googleSheetService.getAvailableSheets(
        googleSheetLink,
      );
  ...
  comparePerformance: async (): Promise<void> => {
    const googleSheetLink =
      'https://docs.google.com/spreadsheets/d/1VPQgrDqbvVhxbjYp5Vv2Korvh2fYCubB/edit';
